...
public sealed record SearchHotelsRequest(string Prompt, int Page = 1, int PageSize = 20);
public sealed record SearchCriteria(GeoLocation Location, decimal MaximumBudget, string Currency);
public sealed record HotelSearchItem(Guid Id, string Name, MoneyDto PricePerNight, double DistanceKm, double Score);
public sealed record SearchCriteriaResponse(GeoLocationDto Location, decimal MaximumBudget, string Currency);
